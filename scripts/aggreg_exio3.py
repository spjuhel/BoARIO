...
def aggreg(exio_path, aggreg_path, sectors_name_path, save_path=None):
    rootLogger.info("Parsing exiobase3 from {}".format(exio_path))
    exio3 = pym.parse_exiobase3(path=exio_path)
    rootLogger.info("Done")
    rootLogger.info("Reading aggregation matrix from sheet 'input' in file {}".format(aggreg_path))
    ...
    rootLogger.info("Done")
    rootLogger.info("Renaming sectors from {}".format(sectors_name_path))
    ...
    rootLogger.info("Saving to {}".format(name))
