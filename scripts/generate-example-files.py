...
def full_mrio_pickle(exio3, save_path=None):
    ...
    scriptLogger.info("Done")
    scriptLogger.info(
        "Saving Full mrio pickle file to {}".format(pathlib.Path(save_path).absolute())
    )
    exio3 = lexico_reindex(exio3)
...
def aggreg_mrio_pickle(full_exio_path, sector_aggregator_path, save_path=None):
    ...
    with exio_path.open("rb") as f:
        ...
        exio3 = pkl.load(f)
    assert isinstance(exio3, IOSystem)
    sec_agg_vec = pd.read_excel(
        sector_aggregator_path, sheet_name="aggreg_input", engine="odf"
    )
    sec_agg_newnames = pd.read_excel(
        sector_aggregator_path,
    ...
    scriptLogger.info(
        "Reading aggregation matrix from sheet 'input' in file {}".format(
            pathlib.Path(sector_aggregator_path).absolute()
        )
    )
    scriptLogger.info(
        "Aggregating from {} to {} sectors".format(
            len(exio3.get_sectors()), len(sec_agg_vec.group.unique())
        )
    )  # type:ignore
...
def params_from_ods(ods_file, monetary, main_inv_dur):
    ...
    mrio_params["capital_ratio_dict"] = (
        df[["Aggregated version sector", "Capital to VA ratio"]]
        .set_index("Aggregated version sector")
        .to_dict()["Capital to VA ratio"]
    )
    mrio_params["inventories_dict"] = (
        df[["Aggregated version sector", "Inventory size (days)"]]
        .set_index("Aggregated version sector")
        .to_dict()["Inventory size (days)"]
    )
    return mrio_params
...
def event_tmpl_from_ods(ods_file):
    ...
    df = pd.read_excel(ods_file)  # type: ignore
    event_params["aff_sectors"] = df.loc[
        (df.Affected == "yes"), "Aggregated version sector"
    ].to_list()
    event_params["rebuilding_sectors"] = (
        df.loc[
            (df["Rebuilding factor"] > 0),
            ["Aggregated version sector", "Rebuilding factor"],
        ]
        .set_index("Aggregated version sector")
        .to_dict()["Rebuilding factor"]
    )
    return event_params


parser = argparse.ArgumentParser(
    description="Build a minimal example for BoARIO, from EXIOBASE3 MRIO table zip file"
)
parser.add_argument(
    "source_path", type=str, help="The str path to the directory with source materials"
)
parser.add_argument(
    "-o",
    "--output",
    type=str,
    help="The path to the example directory to create",
    nargs="?",
...
if __name__ == "__main__":
    args = parser.parse_args()
    scriptLogger.info(
        "Make sure you use the same python environment when you use the minimal example as now."
    )
    ...
    scriptLogger.info(
        "This will create the following directory, with all required files for the minimal example : {}".format(
            output_dir.resolve()
        )
    )
    ...
    if not sec_agg_ods.exists():
        raise FileNotFoundError(
            "Sector aggregator ods file not found - {}".format(sec_agg_ods)
        )
    ...
    if not (output_dir / full_exio_pickle_name).exists():
        ...
        for root, dirs, files in os.walk(args.source_path):
            scriptLogger.info(
                "Looking for Exiobase3 file here {}".format(args.source_path)
            )
            for f in files:
                if regex.match(f):
        ...
        if exio_path is None:
            raise FileNotFoundError(
                "Exiobase file not found in given source directory - {}".format(
                    args.source_path
                )
            )

        scriptLogger.info(
            "Parsing EXIOBASE3 from {} - Note that this takes a few minutes on a powerful laptop. ".format(
                exio_path.resolve()
            )
        )
    ...
    if not (output_dir / minimal_exio_name).exists():
        aggreg_mrio_pickle(
            output_dir / full_exio_pickle_name,
            sector_aggregator_path=sec_agg_ods,
            save_path=output_dir / minimal_exio_name,
        )

    # create params file
    if not (output_dir / params_file_name).exists():
        scriptLogger.info(
            "Generating simulation parameters file : {}".format(
                (output_dir / params_file_name).resolve()
            )
        )
        params = PARAMS
    ...
    if not (output_dir / mrio_params_file_name).exists():
        scriptLogger.info(
            "Generating mrio parameters file : {}".format(
                (output_dir / mrio_params_file_name).resolve()
            )
        )
        mrio_params = params_from_ods(
            params_ods, EXIO3_MONETARY, MAIN_INVENTORY_DURATION
        )
    ...
    if not (output_dir / event_file_name).exists():
        scriptLogger.info(
            "Generating event file : {}".format(
                (output_dir / event_file_name).resolve()
            )
        )
