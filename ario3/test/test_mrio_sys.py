import numpy as np
import ario3.mrio.mrio_module as mrio_sys
from ario3.utils.logger import debug_logger
import pytest
import pickle
...
exio = pym.load_all('./mock_data/Exio_treated')
with open("./mock_data/exio_sectors_params.json", 'r') as fp:
    exio_params = json.load(fp)

with open("./mock_data/params.json", 'r') as fp:
    params = json.load(fp)

result_storage = "./mock_data/results/"
with open("./mock_data/mrio.pkl", 'rb') as fp:
    exio = pickle.load(fp)

test_mrio = mrio_sys.Mrio_System(exio, exio_params, params, result_storage)
...
def test_production_cap():
    ...
    debug_logger.debug("Maximum value in difference: %f (should be 0.0)",(prod_cap - mrio.production_cap).max())
...
def test_production():
    ...
    debug_logger.debug((prod - mrio.production).max())
    ...
    # debug_logger.debug("n sectors: %f", mrio.n_sectors)
    # debug_logger.debug("n regions: %f", mrio.n_regions)
    # debug_logger.debug("Infinite stocks: "+np.array2string(inv_dur[:, np.newaxis]*np.full(check.shape, True)))
    # debug_logger.debug("never limiting or currently limiting stocks: "+np.array2string(check2))
    ...
    # debug_logger.debug("infinite or never limiting or currently limiting stocks: "+np.array2string(final_check))
    # debug_logger.debug("Thresh at idx: "+np.array2string(mrio.matrix_share_thresh[problematic_idx2]))
    # debug_logger.debug("Stock contraigning at idx: "+np.array2string(check[problematic_idx2]))
    # debug_logger.debug("Infinite ? at idx: "+np.array2string((inv_dur[:, np.newaxis]*np.full(check.shape, True))[problematic_idx2]))
